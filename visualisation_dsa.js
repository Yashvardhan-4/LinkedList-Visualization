document.addEventListener('DOMContentLoaded', () => {
    ...
    function generateSteps(operation) {
        ...
        while (p1 < state.poly1.length && p2 < state.poly2.length) {
            ...
            if (node1.power === node2.power) {
                ...
                state.steps.push({ action: 'add_node', from: 'merge', data: { coef: res, power: node1.power }, p1, p2, commentary: `Powers are equal. Adding coefficients (${node1.coef} ${opChar} ${node2.coef} = ${res}).`, codeId, line: 0 });
                p1++; p2++;
    ...
    const resetVisualization = (fullReset = false) => {
        ...
        if (fullReset) {
            ...
            dom.inputSummary.classList.add('hidden');
            dom.polyCoeffForm.innerHTML = ''; dom.polyCoeffForm.classList.add('hidden');
        ...
        clearLog();
        dom.codeSnippetBox.textContent = cppCodeSnippets.NONE;
        showInitialLists();
    };
    ...
    dom.nextPolyStepBtn.onclick = () => { const maxPower = parseInt(dom.maxPowerInput.value); if (isNaN(maxPower) || maxPower < -1) { alert("Please enter a valid power (>= -1 for empty list)."); return; } dom.polyInputStep1.classList.add('hidden'); dom.polyCoeffForm.innerHTML = ''; for (let p = maxPower; p >= 0; p--) { dom.polyCoeffForm.innerHTML += `<div><label>Coef of x^${p}: <input type="number" name="coeff_${p}" step="any" value="0"></label></div>`; } dom.polyCoeffForm.classList.remove('hidden'); dom.submitPolyBtn.classList.remove('hidden'); };
    dom.submitPolyBtn.onclick = () => {
        const maxPower = parseInt(dom.maxPowerInput.value); let arr = [];
        ...
        if (state.currentPolyTarget === 1) {
            state.currentPolyTarget = 2; showInitialLists();
            ...
            dom.inputSummary.classList.remove('hidden');
